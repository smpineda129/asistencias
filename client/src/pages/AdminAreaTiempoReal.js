...
const AdminAreaTiempoReal = () => {
  ...
  const cargarDatos = async () => {
    try {
      ...
      // Cargar datos de tiempo real y InHouses en paralelo
      const [tiempoRealRes, inHousesRes] = await Promise.all([
        attendanceAPI.obtenerEstadoTiempoRealArea(areaId),
        api.get(`/areas/${areaId}/inhouses`)
      ]);
      
      console.log("Datos tiempo real área:", tiempoRealRes.data);
      setDatos(tiempoRealRes.data);
  ...
  // Filtrar usuarios por InHouse
  const filtrarUsuariosPorInHouse = (usuarios) => {
    if (filtroInHouse === 'todos') return usuarios;
    
    const inHouseSeleccionado = inHouses.find(ih => ih._id === filtroInHouse);
    if (!inHouseSeleccionado) return usuarios;
    
    const usuariosInHouseIds = inHouseSeleccionado.usuariosAsignados?.map(u => u._id || u) || [];
    return usuarios.filter(u => usuariosInHouseIds.includes(u.id));
  };

  const usuariosActivosFiltrados = datos ? filtrarUsuariosPorInHouse(datos.usuariosActivos) : [];
  const usuariosInactivosFiltrados = datos ? filtrarUsuariosPorInHouse(datos.usuariosInactivos) : [];
          ...
          <div className="mb-8">
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                ...
                <select
                  value={filtroInHouse}
                  onChange={(e) => setFiltroInHouse(e.target.value)}
                  className="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent text-sm"
                >
                  <option value="todos">Todos los In Houses</option>
                  {inHouses.map((inHouse) => (
                    <option key={inHouse._id} value={inHouse._id}>
                      {inHouse.nombre} ({inHouse.usuariosAsignados?.length || 0} usuarios)
                    </option>
                  ))}
                </select>
              </div>
              ...
              <div className="card mb-8">
                <div className="flex items-center justify-between mb-6">
                  <h2 className="text-2xl font-bold text-gray-800 flex items-center">
                    ...
                    Usuarios Activos ({usuariosActivosFiltrados.length})
                  </h2>
                ...
                {usuariosActivosFiltrados.length === 0 ? (
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full">
                      ...
                      <tbody>
                        {usuariosActivosFiltrados.map((usuario) => (
                          <tr
                            key={usuario.id}
              ...
              <div className="card">
                <div className="flex items-center justify-between mb-6">
                  <h2 className="text-2xl font-bold text-gray-800 flex items-center">
                    ...
                    Usuarios Sin Ingresar ({usuariosInactivosFiltrados.length})
                  </h2>
...
export default AdminAreaTiempoReal;
